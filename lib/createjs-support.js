...
(function (alm) {
    ...
    AnimateUtil.composition = null;
    AnimateUtil.lib = null;
    AnimateUtil.images = null;
    ...
    var Logger = (function () {
        ...
        Logger.verbose = function () {
            ...
            if (Logger.level <= LoggerLevel.Verbose)
                console.log.apply(console, Array.prototype.slice.call(messages.unshift("Verbose: ")));
        };
        Logger.trace = function () {
            ...
            if (Logger.level <= LoggerLevel.Trace)
                console.log.apply(console, Array.prototype.slice.call(messages.unshift("Trace: ")));
        };
        Logger.warn = function () {
            ...
            if (Logger.level <= LoggerLevel.Warn)
                console.log.apply(console, Array.prototype.slice.call(messages.unshift("Warn: ")));
        };
        Logger.error = function () {
            ...
            if (Logger.level <= LoggerLevel.Error)
                console.log.apply(console, Array.prototype.slice.call(messages.unshift("Error: ")));
        };
...
(function (alm) {
    ...
    ScrollWatcher.windowScrollHandler = function (event) {
        ScrollWatcher.apply();
        ScrollWatcher.eventDispatcher.dispatchEvent(new alm.ScrollWatcherEvent(alm.ScrollWatcherEvent.SCROLL, false, false, event, ScrollWatcher.scrollTop, ScrollWatcher.scrollBottom, ScrollWatcher.windowHeight));
    };
...
(function (alm) {
    var ScrollWatcherEvent = (function (_super) {
        ...
        function ScrollWatcherEvent(eventType, bubbles, cancelable, jqueryEvent, scrollTop, scrollBottom, windowHeight) {
            if (bubbles === void 0) { bubbles = false; }
            if (cancelable === void 0) { cancelable = false; }
            if (jqueryEvent === void 0) { jqueryEvent = null; }
...
(function (alm) {
    var AssetLoader = (function () {
        ...
        AssetLoader.initialize = function () {
            ...
            this.eventDispatcher = new createjs.EventDispatcher();
            this.animateImages = alm.AnimateUtil.getImages() || {};
        };
        AssetLoader.load = function () {
            ...
            this.isLoading = true;
            this.initialize();
            alm.Logger.verbose("[AssetLoader] load " + this.loader.getItems(false).length + " files");
            this.setListener();
            this.loader.load();
        };
        ...
        AssetLoader.getAnimateManifest = function (basePath) {
            ...
            var manifest = alm.AnimateUtil.getManifest();
            if (manifest == null)
                return null;
            if (basePath != "") {
    ...
    AssetLoader.loaderFileLoadHandler = function (event) {
        alm.Logger.verbose("[AssetLoader] fileLoad");
        var itemId = event.item.id;
        var result = event.result;
        if (itemId)
            AssetLoader.animateImages[itemId] = result;
    ...
    AssetLoader.loaderErrorHandler = function (event) {
        alm.Logger.error("[AssetLoader] error : ", event);
    ...
    AssetLoader.loaderFileErrorHandler = function (event) {
        alm.Logger.error("[AssetLoader] fileError : ", event);
